...
class salesClass:
    def __init__(self,root):
        ...
        btn_open = Button(self.root, text="Open", command=self.open, font=("times new roman", 15, "bold"),
                            bg="#2196f3", fg="white", cursor="hand2").place(x=590, y=100, width=100, height=28)
    ...
    def search(self):
        if self.var_invoice.get()=="":
            ...
            if self.var_invoice.get() in self.bill_list:
                # fp = open(f'bill/{self.var_invoice.get()}.pdf', 'r')
                # self.bill_area.delete('1.0',END)
                # for i in fp:
                #     self.bill_area.insert(END, i)
                # fp.close()
                os.startfile(f'{self.var_invoice.get()}.pdf')
            else:
