...
class categoryClass:
    def __init__(self,root):
        ...
        lbl_name=Label(self.root, text="Enter Category Nane",font=("goudy old style", 30), bg="white").place(x=50,y=100)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error", "Cactegory already present,try different", parent=self.root)
                else:
                    cur.execute(
